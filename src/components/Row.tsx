...
import { TrashIcon, PencilSquareIcon, DocumentDuplicateIcon, EyeIcon, EyeSlashIcon } from "@heroicons/react/24/outline";
import { readAliasMap, isApuIncompleteDetail } from "../utils/match";
      ...
      >
        ...
        <td className="h-10 px-2 align-middle">
          <div className="flex items-center justify-end gap-1">
            ...
            {onDuplicate && (
              <button aria-label="Duplicar partida" title="Duplicar partida" onClick={() => onDuplicate(row.id)} className="p-1 rounded-md text-slate-300 hover:text-white hover:bg-slate-700/60"><DocumentDuplicateIcon className="h-4 w-4"/></button>
            )}
            <button
              ...
              <td className="h-10 px-2 align-middle">
                <div className="flex items-center justify-end gap-1">
                  <button
                    ...
                    className="p-1 rounded-md text-slate-300 hover:text-white hover:bg-slate-700/60"
                  >
                    {subCollapsed[sid] ? <EyeIcon className="h-4 w-4"/> : <EyeSlashIcon className="h-4 w-4"/>}
                  </button>
                  {onUpdateSubRow && (
                    <button
            ...
            {sIds.length > 0 && !subCollapsed[sid] && (
              <tr className="bg-slate-900/40">
                ...
                <td className="px-3 py-2 align-top" colSpan={5}>
                  {/* Lista de APUs: estilo ajustado */}
                  <div className="grid gap-1 pl-2">
                    {sIds.map((id) => {
                      try {
                        const apu = getApuById(id);
                        const unit = (() => { try { return unitCost(apu, resources).unit || 0; } catch { return 0; } })();
                        const det = (() => { try { return isApuIncompleteDetail(apu); } catch { return { incomplete:false, reasons:[] as string[] }; } })();
                        const incomplete = det.incomplete || !String(apu?.unidadSalida||'').trim();
                        const isZero = unit === 0;
                        const pillClass = incomplete
                          ? 'bg-amber-900/40 border-amber-700/50 text-amber-200'
                          : (isZero ? 'bg-rose-900/30 border-rose-700/50 text-rose-200' : 'bg-slate-800/70 border-slate-700 text-slate-200');
                        const pillTitle = incomplete
                          ? (()=>{
                              const reasons = Array.from(new Set<string>([
                                ...(det.reasons||[]),
                                ...(!String(apu?.unidadSalida||'').trim()? ['sin unidad']: [])
                              ]));
                              return reasons.length? `Incompleto: ${reasons.join(', ')}` : 'Incompleto';
                            })()
                          : (isZero ? 'PU=0 — revisa recursos/secciones' : 'Costo directo unitario');
                        return (
                          <div key={id} className="text-[11px] group rounded-lg border border-slate-700/60 bg-slate-900/50 hover:bg-slate-800/60 transition-colors">
                            <div className="flex items-center justify-between px-3 py-1.5">
                              {/* Izquierda: badge + nombre (clic para ver detalle) */}
                              <button
                                onClick={() => onShowApuDetail && onShowApuDetail(id)}
                                className="min-w-0 flex items-center gap-2 text-left"
                                title={apu?.descripcion}
                                aria-label="Ver detalle del APU"
                              >
                                {(() => {
                                  ...
                                  return label ? (
                                    <span
                                      className="inline-flex items-center px-1.5 py-0.5 rounded-md bg-slate-800/70 border border-slate-700 text-[10px] text-slate-200 font-mono"
                                      aria-hidden="true"
                                    >
                                      {label}
                                    </span>
                                ...
                                <span className="truncate max-w-[48ch] text-slate-200 hover:underline">
                                  {apu?.descripcion}
                                </span>
                              </button>
                              {/* Derecha: costo directo y borrar */}
                              <div className="flex items-center gap-2 pl-2">
                                <span
                                  className={`inline-flex items-center px-1.5 py-0.5 rounded-md border text-[10px] font-mono tabular-nums ${pillClass}`}
                                  title={pillTitle}
                                  aria-label="Costo directo unitario"
                                >
                                  {fmt(unit)}
                                </span>
                                <button
                                  onClick={() => {
                                    ...
                                    const patch = next.length === 0 ? { apuIds: next, _noAutoApu: true } : { apuIds: next };
                                    onUpdateSubRow && onUpdateSubRow(row.id, sid, patch);
                                  }}
                                  className="p-1 rounded-md text-slate-400 hover:text-red-200 hover:bg-red-900/30"
                                  title="Quitar APU"
